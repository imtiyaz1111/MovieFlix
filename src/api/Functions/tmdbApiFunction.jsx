...
export const getAllMovieList = async (type, setItems, setTotalPages) => {
  try {
...
export const getAllTvList = async (type, setItems, setTotalPages) => {
  try {
    const res = await axiosIntance.get(TMDB_ENDPOINTS.getTvList(type));
    if (res) {
      console.log("res", res);
      setItems(res.results || []);
      setTotalPages(res.total_pages || 1);
...
export const getDetails = async (cat, id, setItem) => {
  try {
    const res = await axiosIntance.get(TMDB_ENDPOINTS.detail(cat, id));
    if (res) {
      setItem(res);
      window.scrollTo(0, 0);
    }
